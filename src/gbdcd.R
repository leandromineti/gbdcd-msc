## Desenvolvimento do codigo do RJMCMC/Gaussian BDCD
## utilizando dados de regressao univariado
## Leandro Mineti/Marcelo A Costa
## revisado em 20/04/2020

# Define gbdcd univariate regression function
gbdcd <- function(y,
                  X,
                  viz,
                  n_iterations = 100000,
                  burn_in = 50000,
                  c = 0.35,
                  prior_coeffs_mu = rep(0, 2), # Media a priori para Betas
                  prior_sigma = sqrt(2), # Valor inicial p/ sigma
                  lambda = 0.001,
		  plot = T) { # Parametros a priori Cov(Betas)
  ...
  invLambdaIdentityPrior <- diag(rep(1 / lambda, 2))
  ...
  for (i in 1:n_iterations)
  {
    ...
    if (step_choice == "birth") {
      ...
      mat_coeffs[new_center, ] <- rmvnorm(1,
        mean = coeffs_mean_proposed,
        sigma = coeffs_cov_proposed
      )

      phi <- dmvnorm(mat_coeffs[new_center, ],
        mean = coeffs_mean_proposed,
        sigma = coeffs_cov_proposed,
        log = TRUE
      )

      phi_k_plus_1 <- dmvnorm(mat_coeffs[new_center, ],
        mean = prior_coeffs_mu,
        sigma = sigma2 * invLambdaIdentityPrior,
        log = TRUE
      )
    ...
    if (step_choice == "death") {
      ...
      phi <- dmvnorm(mat_coeffs[center, ],
        mean = coeffs_mean_proposed,
        sigma = coeffs_cov_proposed,
        log = TRUE
      )

      phi_k_minus_1 <- dmvnorm(mat_coeffs[center, ],
        mean = prior_coeffs_mu,
        sigma = sigma2 * invLambdaIdentityPrior,
        log = TRUE
      )
    ...
    if (step_choice == "update") {
      ...
      for (center_update in cluster_centers) {
        ...
        mat_coeffs[center_update, ] <- rmvnorm(1,
          mean = coeffs_mean_proposed,
          sigma = coeffs_cov_proposed
        )
      }
